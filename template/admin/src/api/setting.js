 ...
 * @param {Number} param id {Number} 身份id
 */
 ...
 * @param {Number} param id {Number} 身份id
 */
export function roleDelFromApi(id) {
  return request({
...
export function roleSetStatusApi(data) {
  return request({
    url: `/system/role/set_status/${data.id}/${data.status}`,
    method: 'POST',
  });
}
...
export function fileUpload(data) {
  return request({
    url: `/system/user/uploadImage`,
    method: 'post',
    headers: {
      'Authorization': getCookies('token'),
      'content-type': 'multipart/form-data;' + getCookies('token'),
    },
    data,
  });
}
