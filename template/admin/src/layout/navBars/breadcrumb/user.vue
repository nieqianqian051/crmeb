  ...
  <div class="layout-navbars-breadcrumb-user" :style="{ flex: layoutUserFlexNum }">
    ...
    <el-dropdown :show-timeout="70" @command="onDropdownCommand">
      <span class="layout-navbars-breadcrumb-user-link">
        <img :src="getUserInfos.headPic" class="layout-navbars-breadcrumb-user-link-photo mr5" />
  ...
  methods: {
    ...
    onDropdownCommand(path) {
      if (path === 'logOut') {
		...
		setTimeout(() => {
			let obj = this.$store.state.userInfo.userInfo
			getUserQrcode(obj.id).then(res=>{
				this.$router.push({ name: 'systemUser', params: { data: res }});
			})
		},150);
