  ...
  <div>
    ...
    <el-card :bordered="false" shadow="never" class="ivu-mt">
      <vxe-table
        ...
        row-id="id"
      >
        <vxe-table-column field="menuName" tree-node title="按钮名称" min-width="100"></vxe-table-column>
        <vxe-table-column field="menuPath" title="类型" min-width="240" tooltip="true">
          <template v-slot="{ row }">
            <span v-if="row.authType == 1">菜单：{{ row.menuPath }}</span>
            <span v-if="row.authType == 3">按钮</span>
            <span v-if="row.authType == 2">数据权限</span>
          </template>
        </vxe-table-column>
        ...
        <vxe-table-column field="flag" title="是否显示" width="150">
          <template v-slot="{ row }">
            <el-switch
              :active-value="1"
              :inactive-value="0"
              v-model="row.isShowPath"
              :value="row.isShowPath"
    ...
    menusDetails(id) {
      menusDetailsApi(id)
        .then(async (res) => {
          this.formValidate = {
            ...res.data,
            api_url: res.data.apiUrl,
            auth_type: res.data.authType,
            is_del: res.data.isDel,
            is_header: res.data.isHeader,
            is_show: res.data.isShow,
            is_show_path: res.data.isShowPath,
            menu_name: res.data.menuName,
            menu_path: res.data.menuPath,
            unique_auth: res.data.uniqueAuth,
            path: res.data.path == "" ? "" : res.data.path.split("/")
          }
          this.$refs.menusFrom.modals = true;
        })
        .catch((res) => {
