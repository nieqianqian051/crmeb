        ...
        <div class="table-box">
          ...
          >
            ...
            <el-table-column label="模板类型" min-width="130">
              <template slot-scope="scope">
                <el-tag size="medium" v-if="scope.row.status == 1">首页</el-tag>
                <el-tag size="medium" 
					v-if="scope.row.isDiy == 1 && scope.row.status == 0" 
					class="mr10">{{scope.row.typeName}}</el-tag>
                <el-tag size="medium" 
				v-if="scope.row.isDiy == 0 && scope.row.status == 0">
				{{scope.row.typeName}}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="添加时间" min-width="130">
              <template slot-scope="scope">
                <span>{{ scope.row.addTime | timestampToTime }}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" fixed="right" width="210">
              <template slot-scope="scope">
                <div style="display: inline-block" v-if="scope.row.status || scope.row.isDiy" @click="edit(scope.row)">
                  <a
                    v-if="scope.row.isDiy === 1"
                    class="target"
                    ref="target"
                    :href="`${url}${$routeProStr}/setting/pages/diy_index?id=${scope.row.id}&name=${
                      scope.row.templateName || 'moren'
                    }`"
                    target="_blank"
                  >
                    编辑</a
                ...
                <el-divider
                  direction="vertical"
                  v-if="(scope.row.status || scope.row.isDiy) && scope.row.id != 1 && scope.row.status != 1"
                />
                ...
                <el-divider
                  direction="vertical"
                  v-if="(scope.row.id != 1 && scope.row.status != 1) || scope.row.isDiy"
                />
                <div style="display: inline-block" v-if="scope.row.isDiy">
                  <a @click="preview(scope.row, scope.$index)">预览</a>
                </div>
                <el-divider direction="vertical" v-if="scope.row.isDiy && scope.row.status != 1" />
...
export default {
  ...
  data() {
    return {
      ...
      iframeUrl: '',
      modal: false,
      BaseURL: Setting.apiBaseURL.replace(/api/, ''),
      cardShow: 0,
      loadingExist: false,
      isDiy: 1,
      qrcodeImg: '',
  ...
  created() {
    ...
    //this.iframeUrl = `${location.origin}/pages/index/index?mdType=iframeWindow`;
	  this.iframeUrl = `http://crmeb.lycrps.com/pages/index/index?mdType=iframeWindow`;
  },
  mounted() {},
  methods: {
    cancel() {
